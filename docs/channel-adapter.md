...
- inbound file/video: documents and videos are downloaded, AES-128-ECB decrypted, and uploaded as session attachments (file keeps its name; video → `video.mp4`); over the 25 MiB cap they are skipped
- outbound voice (DM replies to a voice note): the reply is synthesized via TTS as **Ogg/Opus @ 48 kHz** (`audio/ogg`), uploaded to the C2C CDN (`getuploadurl` media_type=VOICE + AES-128-ECB encrypt, download ref read from the `x-encrypted-param` upload-response header), and sent as a voice item with `encode_type: 8` (Ogg/Opus) — the format WeChat renders for bot-sent voice (SILK is the QQ format and is silently dropped by iLink on the bot→user direction). **Off by default** (`OPENHERMIT_WECHAT_VOICE_REPLY=1` to enable): iLink silently drops bot→user VOICE (accepted with `ret=0` but never rendered — confirmed live), so voice notes are answered with text
- outbound media (agent → user): `attachment` events are downloaded, encrypted, uploaded to the C2C CDN, and sent as the matching item — images (`image_item`), video (`video_item`), or any other file as a `file_item`. iLink delivers these (unlike voice). Outbound media over 8 MiB is skipped (slow upload link); an optional caption is a leading text item. Inbound file/video are not handled yet
- no group filtering
