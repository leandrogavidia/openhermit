...
export class VexaWebhookReceiver {
  ...
  async handleWebhook(req: WebhookRequest): Promise<WebhookResponse> {
    ...
    // A verified, well-formed delivery proves the webhook wiring works; clear
    // any stale error the UI may be showing.
    this.reportRuntimeError?.(null);

    const normalized = normalizeEvent(event);
    if (!normalized) {
      // Not a completion event we act on (joining, active, chat, …): ack so
      // Vexa stops retrying, and ignore.
      return ack();
    }
