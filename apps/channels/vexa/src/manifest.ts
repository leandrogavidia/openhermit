 ...
 * `channelPackages` in gateway config. Webhook-only: no outbound surface and
 * no background loop — it exposes a `handleWebhook` the gateway routes Vexa
 * events to. A finished meeting triggers an owner-scoped agent turn that
 * captures the transcript into memory via the `vexa-meetings` skill + the
 * Vexa MCP tools (`mcp__vexa__*`). See `docs/vexa-meetings.md`.
 */
  ...
  secretKeys: [
    {
      ...
      placeholder: 'shared secret you also set in Vexa (PUT /user/webhook)',
    },
  ],
  ...
  configFields: [
    {
      ...
      label: 'Webhook URL',
      help:
        'Register this URL in Vexa (PUT /user/webhook with your webhook_secret) so ' +
        'finished meetings are captured into memory automatically.',
    },
  ],
