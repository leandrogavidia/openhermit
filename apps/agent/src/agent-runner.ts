...
export class AgentRunner implements SessionRuntime {
  ...
  private async resolveSessionUser(
    ...
    // event (a meeting ending) triggered the turn, and there is no human
    // sender to attribute it to. Resolve to the agent's owner so memory writes
    // and tool access are owner-scoped. Honoured only on NON-interactive
    // sessions so a human-facing channel can never escalate to owner by
    // forging the flag — the bridge that sets it never opens interactive
    // sessions.
